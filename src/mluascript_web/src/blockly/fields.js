...
const OriginalFieldTextInput = Blockly.FieldTextInput

function applyForeignObjectEditor(instance) {
  if (!instance.foreignObject_) {
    instance.foreignObject_ = document.createElementNS('http://www.w3.org/2000/svg', 'foreignObject')
    instance.fieldGroup_.appendChild(instance.foreignObject_)
  }
  
  const updateRect = () => {
    if (!instance.foreignObject_) return
    const rect = instance.borderRect_
    if (rect) {
      instance.foreignObject_.setAttribute('x', rect.getAttribute('x') || 0)
      instance.foreignObject_.setAttribute('y', rect.getAttribute('y') || 0)
      instance.foreignObject_.setAttribute('width', rect.getAttribute('width') || instance.size_.width)
      instance.foreignObject_.setAttribute('height', rect.getAttribute('height') || instance.size_.height)
    } else {
      instance.foreignObject_.setAttribute('x', 0)
      instance.foreignObject_.setAttribute('y', -(instance.size_.height || 24) / 2)
      instance.foreignObject_.setAttribute('width', instance.size_.width || 40)
      instance.foreignObject_.setAttribute('height', instance.size_.height || 24)
    }
  }
  
  updateRect()
  
  if (instance.htmlInput_ && instance.htmlInput_.parentNode !== instance.foreignObject_) {
    instance.foreignObject_.appendChild(instance.htmlInput_)
    instance.htmlInput_.classList.add('blockly-embedded-input')
    instance.htmlInput_.addEventListener('mousedown', (evt) => evt.stopPropagation())
    setTimeout(() => {
      if (instance.htmlInput_) {
        instance.htmlInput_.focus()
        instance.htmlInput_.select()
      }
    }, 0)
  }
  ...
  const originalResize = instance.resizeEditor_
  instance.resizeEditor_ = function() {
    if (originalResize) originalResize.call(this)
    updateRect()
  }
}

const originalNumberShowEditor = OriginalFieldNumber.prototype.showEditor_
OriginalFieldNumber.prototype.showEditor_ = function(e) {
  originalNumberShowEditor.call(this, e)
  applyForeignObjectEditor(this)
}

const originalNumberWidgetDispose = OriginalFieldNumber.prototype.widgetDispose_
OriginalFieldNumber.prototype.widgetDispose_ = function() {
  originalNumberWidgetDispose.call(this)
  if (this.foreignObject_) {
    this.foreignObject_.remove()
    this.foreignObject_ = null
  }
}

const originalTextShowEditor = OriginalFieldTextInput.prototype.showEditor_
OriginalFieldTextInput.prototype.showEditor_ = function(e) {
  originalTextShowEditor.call(this, e)
  applyForeignObjectEditor(this)
}

const originalTextWidgetDispose = OriginalFieldTextInput.prototype.widgetDispose_
OriginalFieldTextInput.prototype.widgetDispose_ = function() {
  originalTextWidgetDispose.call(this)
  if (this.foreignObject_) {
    this.foreignObject_.remove()
    this.foreignObject_ = null
  }
