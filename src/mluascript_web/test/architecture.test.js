...
test('the editor uses file tabs, right-click tree actions, and compact top menus', () => {
  ...
  assert.match(editorSource, /selectedFileIsImage/)
  assert.match(editorSource, /return \/\\\.\(png\|jpe\?g\|gif\|webp\|bmp\|svg\|avif\|ico\)\$\/i\.test\(path\) \? 'image' : 'binary'/)
  assert.match(editorSource, /image: '图片'/)
  assert.match(editorSource, /class="image-preview"/)
  ...
  assert.match(editorSource, /label: editorMenuLabel\('识图调试…'\)/)
  assert.match(editorSource, /await openTaskById\(task\.taskId\)/)
  assert.match(editorSource, /label: '在截图工具中打开'/)
  assert.match(editorSource, /label: '作为识图模板'/)
