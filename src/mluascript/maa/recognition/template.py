...
def find_template(
    ...
    templates = [] if template is None else ([template] if isinstance(template, str) else template)
    param = JTemplateMatch(template=[resolve_resource_reference(context, item) for item in templates])
    if roi is not None:
